...
namespace gr {
  namespace ft8 {
    ...
    void 
    encoder_impl::init_gaussian_coefs()
    {
      if (s_coefs_initialized) return;

      s_gaussian_coefs = gr::filter::firdes::gaussian(
        1.0f, //gain
    ...
    std::vector<float>
    encoder_impl::apply_fir_filter(const std::vector<float>& fsk_signal)
    {
      if(!d_fir_filter){
